...
with open("network.xml", "w") as f:
    ...
    for line in lines.values():
        ...
        for i in range(len(st)-1):
            ...
            f.write(f'    <link id="l{link_id}" from="{a}" to="{b}" length="{dist}" freespeed="20" capacity="2000" permlanes="1" modes="pt"/>\n')
            link_id += 1
...
with open("transitSchedule.xml", "w") as f:
    ...
    for nid, (x,y) in node_dict.items():
        f.write(f'    <stopFacility id="stop_{nid}" x="{x}" y="{y}" linkRefId="{nid}" name="{nid}"/>\n')
    ...
    for line_name, data in lines.items():
        ...
        f.write(f'      <transportMode>{data["mode"]}</transportMode>\n')

        f.write('      <routeProfile>\n')
        for s in data["stations"]:
            f.write(f'        <stop refId="stop_{s}" departureOffset="00:00:00"/>\n')
        f.write('      </routeProfile>\n')

        f.write('      <departures>\n')
        for i in range(0, 1440, data["freq"]):
            if 300 <= i <= 1380:  # PT active between 05:00–23:00
                hh = i // 60
                mm = i % 60
                f.write(f'        <departure id="d_{line_name}_{i}" departureTime="{hh:02d}:{mm:02d}:00"/>\n')
        f.write('      </departures>\n')
...
with open("population.xml", "w") as f:
    ...
    with open("demand_15min.csv") as dfile:
        ...
        for row in reader[:1000]:  # sample
            ...
            f.write('    <plan>\n')
            f.write(f'      <activity type="home" x="{fx}" y="{fy}" endTime="{hh:02d}:{mm:02d}:00"/>\n')
            f.write('      <leg mode="pt"/>\n')
